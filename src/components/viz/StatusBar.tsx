...
export function StatusBar({ html }: { html: string }) {
  return (
    ...
    >
      <span aria-hidden className="select-none pt-px font-mono text-sm text-primary">
        ›
      </span>
      <RichText
        html={html}
        className="min-h-5 flex-1 font-mono text-[13px] leading-relaxed text-text [&_code]:rounded-[2px] [&_code]:bg-surface [&_code]:px-1.5 [&_code]:py-0.5 [&_code]:text-xs [&_code]:text-primary"
      />
    </div>
  )
}
